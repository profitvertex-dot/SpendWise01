    ...
    <div style={{minHeight:'100vh', background:'#FFF5F7', fontFamily:'system-ui', paddingBottom:90}}>
      <div style={{maxWidth:440, margin:'0 auto', padding:16}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
          <div><h1 style={{margin:0, fontSize:32, fontWeight:900}}>SpendWise</h1><p style={{margin:0, color:'#8B8B8B', fontSize:14}}>Track smart, spend smarter ✨</p></div>
        ...
        <div style={{background:'#fff', borderRadius:22, padding:16, marginTop:14}}>
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
          ...
          <div style={{height:12, background:'#F0F0F0', borderRadius:10, marginTop:12, overflow:'hidden'}}>
            <div style={{height:'100%', width:`${pct}%`, borderRadius:10, background: pct>=100? '#FF4D4F' : pct>=80? '#FF9E42' : 'linear-gradient(90deg,#7B5CFF,#4A7BF7)'}}/>
          </div>
        ...
        {total>0 && (
          <div style={{background:'#fff', borderRadius:22, padding:16, marginTop:14}}>
            <h3 style={{margin:'0 0 12px 0', fontSize:16}}>Breakdown</h3>
              ...
              {byCat.map(([c,sum])=>{
                ...
                return (
                  <div key={c} style={{display:'flex', justifyContent:'space-between', alignItems:'center', background:catMeta[c].bg, padding:'12px 14px', borderRadius:14}}>
                    <div style={{display:'flex', gap:10, alignItems:'center'}}><span>{catMeta[c].emoji}</span><b style={{fontSize:13}}>{c}</b> <span style={{fontSize:12, color:'#666'}}>₹{sum}</span></div>
        ...
        <div style={{marginTop:14, display:'grid', gap:8}}>
          {filtered.slice(0,20).map(e=> (
            <div key={e.id} style={{background:'#fff', padding:'12px 14px', borderRadius:14, display:'flex', justifyContent:'space-between'}}>
              <span>{catMeta[e.cat].emoji} {e.title} • <small style={{color:'#888'}}>{e.dateISO}</small></span><span><b>₹{e.amount}</b> <span onClick={()=> del(e.id)} style={{color:'#ff6b6b', cursor:'pointer', marginLeft:8}}>✕</span></span>
            </div>
          ))}
        </div>
      </div>
