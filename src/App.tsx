...
function SwipeRow({e, onDelete, cardBg, textColor}: any){
  ...
  return (
    <div style={{position:'relative', overflow:'hidden', borderRadius:14}}>
      ...
      <div onTouchStart={(ev:any)=> startX.current = ev.touches[0].clientX}
...
export default function App(){
  ...
  return(
    <div style={{minHeight:'100vh', background: dark?'#000':'#FFF1F3', color:textColor, fontFamily:'Inter', paddingBottom:80, boxSizing:'border-box'}}>
      <div style={{maxWidth:400, margin:'0 auto', padding:16, boxSizing:'border-box'}}>
        {page==='home' && (
          <>
            <div style={{display:'flex', justifyContent:'space-between'}}><h1 style={{margin:0, fontSize:28, fontWeight:900}}>SpendWise</h1><button onClick={()=> setDark(!dark)} style={{width:40,height:40,borderRadius:12,border:0,background:cardBg}}>{dark?'☀️':'🌙'}</button></div>
            <div style={{background:'linear-gradient(135deg,#7A5CFA,#4D7CFF)', borderRadius:28, padding:22, marginTop:16, color:'#fff'}}><p style={{margin:0,fontSize:10,opacity:0.8}}>TOTAL / ₹{budget}</p><h1 style={{margin:'6px 0',fontSize:38,fontWeight:900}}>₹{total.toLocaleString()}</h1></div>
            <div style={{marginTop:14,display:'grid',gap:10}}>{expenses.slice(0,8).map(e=> <SwipeRow key={e.id} e={e} cardBg={cardBg} textColor={textColor} onDelete={()=> setExpenses(p=> p.filter(x=> x.id!==e.id))} />)}</div>
        ...
        {page==='history' && (
          <>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}><h2 style={{margin:0}}>Calendar 📅</h2><div style={{display:'flex',gap:8}}><button onClick={()=> setCalDate(new Date(year, month-1,1))} style={{padding:'6px 10px',borderRadius:8,border:0,background:inputBg}}>‹</button><span style={{fontWeight:800,fontSize:13}}>{calDate.toLocaleString('en-IN',{month:'short', year:'numeric'})}</span><button onClick={()=> setCalDate(new Date(year, month+1,1))} style={{padding:'6px 10px',borderRadius:8,border:0,background:inputBg}}>›</button></div></div>
            <div style={{background:cardBg, borderRadius:20, padding:12, marginTop:14}}><div style={{display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:4, textAlign:'center', fontSize:11, opacity:0.5, marginBottom:8}}><span>Su</span><span>Mo</span><span>Tu</span><span>We</span><span>Th</span><span>Fr</span><span>Sa</span></div><div style={{display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:6}}>{days.map((d,i)=>{ if(d===null) return <div key={'e'+i}/>; const iso=`${year}-${String(month+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`; const amt=dayTotals.get(iso); const isSel=selected===iso; return <button key={iso} onClick={()=> setSelected(iso)} style={{aspectRatio:'1', borderRadius:12, border:isSel?'2px solid #7A5CFA':'0', background: amt? (isSel?'#7A5CFA':inputBg):'transparent', color:isSel?'#fff':textColor, fontWeight: amt?800:400, fontSize:12, position:'relative'}}>{d}{amt && <span style={{position:'absolute', bottom:2, left:'50%', transform:'translateX(-50%)', fontSize:7, color:isSel?'#fff':'#7A5CFA'}}>₹{amt>999? `${Math.round(amt/1000)}k`:amt}</span>}</button> })}</div></div>
            {selected? <><h3 style={{margin:'12px 0 8px', fontSize:14}}>{selected} — ₹{(dayTotals.get(selected)||0).toLocaleString()}</h3><div style={{display:'grid', gap:8}}>{expenses.filter(e=> e.dateISO===selected).map(e=> <SwipeRow key={e.id} e={e} cardBg={cardBg} textColor={textColor} onDelete={()=> setExpenses(p=> p.filter(x=> x.id!==e.id))}/> )}{expenses.filter(e=> e.dateISO===selected).length===0 && <p style={{opacity:0.5}}>Is din kuch nahi 🎉</p>}</div><button onClick={()=> setSelected(null)} style={{marginTop:10, padding:8, border:0, background:'transparent', color:'#7A5CFA'}}>← Back to all</button></> : <><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search..." style={{width:'100%', marginTop:12, padding:14, borderRadius:12, border:0, background:inputBg, color:textColor, boxSizing:'border-box', display:'block'}}/><div style={{marginTop:12, display:'grid', gap:8}}>{filtered.map(e=> <SwipeRow key={e.id} e={e} cardBg={cardBg} textColor={textColor} onDelete={()=> setExpenses(p=> p.filter(x=> x.id!==e.id))}/>)}</div></>}
          </>
        )}
        {page==='stats' && (<><h2>Analytics 📊</h2><div style={{background:cardBg,borderRadius:20,padding:16,marginTop:14}}>{byCat.map(([c,s])=> <div key={c} style={{marginBottom:12}}><div style={{display:'flex',justifyContent:'space-between'}}><span>{catMeta[c].emoji} {c}</span><b>₹{s}</b></div><div style={{height:8,background:inputBg,borderRadius:10,marginTop:4}}><div style={{width:`${(s/total)*100}%`,height:'100%',background:catMeta[c].color}}/></div></div>)}</div></>)}
        {page==='settings' && (<><h2>Settings ⚙️</h2><div style={{background:cardBg,borderRadius:20,padding:16,marginTop:14,display:'grid',gap:12}}><div style={{display:'flex',gap:8}}><input type="number" value={budget} onChange={e=> setBudget(Number(e.target.value))} style={{flex:1,padding:12,borderRadius:12,border:0,background:inputBg,color:textColor, boxSizing:'border-box'}}/><span>Budget</span></div><button onClick={()=> setDark(!dark)} style={{padding:12,borderRadius:12,border:0,background:inputBg,color:textColor}}>Dark: {dark?'On':'Off'}</button><button onClick={()=> {if(confirm('Delete all?')) setExpenses([])}} style={{padding:12,borderRadius:12,border:'1px solid #FF3B30',background:'transparent',color:'#FF3B30'}}>Clear All</button></div></>)}
      </div>
      <div style={{position:'fixed',bottom:0,left:0,right:0, background:cardBg, borderTop:'1px solid #eee', display:'flex', maxWidth:400, margin:'0 auto'}}>
